...
#if also counting reads mapped to gbins, input bedfile of reads mapped to genomic bins and genome bin size
if len(sys.argv) > 1:
	gbin_bed_file = sys.argv[2]
	gbin_size = sys.argv[3]

print("Starting:", i)

for l in levels:
	print("Handling ags for:", i, l)
	out_counts_file = i.split(".fastq.gz.ANNOTATED.ci2_and_unq.ag.txt")[0] + "." + l + "." + gbin_size + "gbin.ag.counts.txt"
	resolved_ag_df, res_stats_df  = load_ag_dat(i, l, save=True)

	#count reads for each  ag:
	ag_counts_df = count_rep_gbin_reads(resolved_ag_df, gbin_bed_file)
